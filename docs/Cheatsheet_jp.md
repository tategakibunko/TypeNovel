...
現在の作業ディレクトリに初期設定の`tnconfig.json`を作成します。すでに存在する場合は、なにもしません。

### --format

出力形式を指定します。`html`（初期値）もしくは`text`を指定できます。
...
ブロックレベルのマークアップには以下のようにして`制約(constraint)`を課すことが出来ます。
...
しかしもし出力されるHTMLに`data-time="?深夜"`という情報が（アプリ側の都合などで）欲しくて、しかも`time`制約については警告されたくない、と言った場合、`無視される制約`は非常に便利です。
